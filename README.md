...
**MRRA（Multi-Radar Reference Analysis）** 是本系统实现的多雷达误差分析核心算法：

**算法流程**：
1. **轨迹段分割**：按时间窗口将连续轨迹分割为若干段
2. **匹配组构建**：基于空间（网格划分）和时间（时间窗口）进行轨迹点匹配
3. **系统误差估计**：使用网格搜索优化方位角和距离偏差
4. **误差修正**：根据估计的偏差值对原始轨迹进行修正

**配置参数**：
| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| grid_resolution | 网格分辨率（度） | 0.1 |
| time_window | 时间窗口（秒） | 60 |
| match_distance_threshold | 匹配距离阈值（度） | 0.15 |
| min_track_points | 最小航迹点数 | 10 |
| cost_weights | 代价函数权重（方差/方位角/距离/俯仰角） | 可配置 |
| optimization_steps | 方位角优化步长列表 | [0.1, 0.01] |
| range_optimization_steps | 距离优化步长列表 | [1000, 500, 100] |
| max_match_groups | 最大匹配组数 | 10000 |

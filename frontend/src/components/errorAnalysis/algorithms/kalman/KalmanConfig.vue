          ...
          <div class="subsection">
            <div class="form-row">
              <div class="form-group">
                <label>过程噪声协方差</label>
                <input v-model.number="localConfig.process_noise" type="number" step="0.01" min="0.001" max="10" class="form-input" :disabled="disabled" />
              </div>
              <div class="form-group">
                <label>测量噪声协方差</label>
                <input v-model.number="localConfig.measurement_noise" type="number" step="0.1" min="0.01" max="100" class="form-input" :disabled="disabled" />
              </div>
            </div>
            <div class="form-group">
              <label>初始不确定性</label>
              <input v-model.number="localConfig.initial_uncertainty" type="number" step="10" min="1" max="10000" class="form-input" :disabled="disabled" />
            </div>
          </div>
        </div>
      </div>
...
const descMap: Record<string, string> = { standard: '默认参数', smooth: '更平滑的结果', responsive: '更快响应变化' }
